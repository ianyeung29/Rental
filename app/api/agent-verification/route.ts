...
export async function GET() {
  ...
  try {
    ...
    const rows = await db.query(`
      SELECT license_state, license_number, brokerage, verification_submitted_at,
...
export async function POST(request: Request) {
  ...
  let body: { licenseState?: unknown; licenseNumber?: unknown; brokerage?: unknown; displayNameZh?: unknown; displayNameEn?: unknown };
  try {
    ...
    const result = await db.transaction((tx) => [
      tx.query(`
        INSERT INTO rental_agent_profiles (
          ...
          verification_submitted_at, verification_reviewed_at, verification_note, updated_at
        ) VALUES ($1, $2, $3, $4, $5, $6, $7, '[]'::jsonb, '["zh", "en"]'::jsonb, FALSE, FALSE, FALSE, NOW(), NULL, '', NOW())
        ON CONFLICT (user_id) DO UPDATE SET
          ...
          verification_note = '',
          updated_at = NOW()
        RETURNING license_state, license_number, brokerage, verification_submitted_at,
                  verification_reviewed_at, verification_note
      `, [profileId, context.user.id, displayNameZh, displayNameEn, brokerage, licenseState, licenseNumber]),
