...
const ALLOWED_SORT_MODES = new Set(["fit", "price", "fresh"]);
...
function normalizeSnapshot(body: Record<string, unknown>) {
  ...
  return { location, maxPrice, rentalType, moveIn, activeFeatures, sortMode };
}
...
export async function GET() {
  try {
    ...
    const rows = await sql!.query("SELECT location, max_price, rental_type, move_in, features, sort_mode, updated_at FROM rental_saved_searches WHERE user_id = $1 LIMIT 1", [result.user.id]);
...
export async function PUT(request: Request) {
  try {
    ...
    const rows = await sql!.query(`
      INSERT INTO rental_saved_searches (user_id, location, max_price, rental_type, move_in, features, sort_mode)
      VALUES ($1, $2, NULLIF($3, '')::numeric, $4, $5, $6::jsonb, $7)
      ON CONFLICT (user_id) DO UPDATE SET
        ...
        updated_at = NOW()
      RETURNING location, max_price, rental_type, move_in, features, sort_mode, updated_at
    `, [result.user.id, snapshot.location, snapshot.maxPrice, snapshot.rentalType, snapshot.moveIn, JSON.stringify(snapshot.activeFeatures), snapshot.sortMode]);
