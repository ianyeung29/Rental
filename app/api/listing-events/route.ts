...
export async function POST(request: Request) {
  ...
  try {
    ...
    const listingRows = await sql.query("SELECT id FROM rental_listings WHERE id = $1 AND status = 'published' AND (expires_on IS NULL OR expires_on >= CURRENT_DATE) LIMIT 1", [listingId]);
