...
const quickDestinations = [
  { zh: "法拉盛市中心", en: "Downtown Flushing" },
  { zh: "布鲁克林八大道", en: "Brooklyn 8th Avenue" },
  { zh: "附近中文超市", en: "a nearby Chinese supermarket" },
];
...
export default function CommuteEstimator({ locale, areaZh, areaEn }: { locale: Locale; areaZh: string; areaEn: string }) {
  ...
  return <section className="commute-panel" aria-labelledby="commute-panel-title">
    <div className="commute-heading"><div><span className="section-label">AREA CONTEXT</span><h3 id="commute-panel-title">{zh ? "估算到目的地的时间" : "Estimate a route from this area"}</h3><p>{zh ? "输入学校、工作地点、商圈或超市；只按公开大致区域估算。" : "Enter a school, workplace, commercial district, or supermarket; estimates use only the public approximate area."}</p></div></div>
    ...
    <div className="commute-quick-options" aria-label={zh ? "常用目的地" : "Common destinations"}>{quickDestinations.map((item) => <button className="commute-quick-option" type="button" key={item.en} onClick={() => { setDestination(zh ? item.zh : item.en); setError(""); }}>{zh ? item.zh : item.en}</button>)}</div>
    {error && <p className="commute-error" role="alert">{error}</p>}
    {result && <div className={`commute-result ${result.source === "google" ? "ready" : "empty"}`} role="status"><div><strong>{result.minutes ? (zh ? `约 ${result.minutes} 分钟` : `About ${result.minutes} min`) : (zh ? "暂时没有路线时间" : "No route time yet")}</strong><span>{result.destination}</span></div><p>{result.note}</p>{result.cached && <small>{zh ? "已使用缓存，没有重复查询地图。" : "Cached result; no new map lookup was made."}</small>}</div>}
  </section>;
}
