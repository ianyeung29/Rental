...
type UsageResult = {
  ...
  monitoring: {
    ...
    month: { openaiEstimatedCostUsd: number; googlePlacesCalls: number; googleRoutesCalls: number; blockedRequests: number };
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main">
      ...
      {loading && !result ? <div className="admin-application-skeletons"><span /><span /></div> : result && <>
        <section className="admin-usage-summary" aria-label="Usage totals"><div><span>请求 / Requests</span><strong>{formatNumber(totals.requests)}</strong></div><div><span>AI tokens</span><strong>{formatNumber(totals.inputTokens + totals.outputTokens)}</strong></div><div><span>Places calls</span><strong>{formatNumber(totals.placesCalls)} <small>/ {formatNumber(result.pricing.googlePlacesFreeMonthly)} free/mo · {formatUnitCost(result.pricing.googlePlacesCostPerCall)} / call after free quota</small></strong></div><div><span>Routes calls</span><strong>{formatNumber(totals.routeCalls)} <small>/ {formatNumber(result.pricing.googleRoutesFreeMonthly)} free/mo · {formatUnitCost(result.pricing.googleRoutesCostPerCall)} / call after free quota</small></strong></div><div><span>Gross estimate</span><strong>{formatCost(totals.grossCostUsd)}</strong></div></section>
