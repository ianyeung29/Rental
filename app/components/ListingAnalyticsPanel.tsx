...
export default function ListingAnalyticsPanel({ locale }: { locale: Locale }) {
  ...
  const listingRecommendation = (listing: AnalyticsListing) => {
    if (listing.views < 12) return zh ? "优先补充清晰首图并精简标题，让搜索结果更容易被点开。" : "Improve the cover photo and shorten the title to earn more search-result opens.";
    if (listing.saves === 0) return zh ? "已有浏览但没有收藏；请复核租金、照片顺序和前三个房源特点。" : "People are viewing but not saving; review rent, photo order, and the first three features.";
    if (listing.contacts === 0 && listing.saves > 0) return zh ? "租客有兴趣但尚未联系；建议确认仍可出租，并补充看房时段。" : "Renters are interested but not contacting; confirm availability and add clear tour windows.";
    if (listing.inquiries > 0) return zh ? "已有咨询。优先快速回复，并确认房源仍然可租。" : "Inquiries are arriving. Reply quickly and keep availability confirmed.";
    return zh ? "继续保持资料更新；收到更多互动后会给出更具体建议。" : "Keep the listing current; guidance will become more specific as engagement grows.";
  };
