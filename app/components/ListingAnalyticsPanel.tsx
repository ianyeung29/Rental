...
export default function ListingAnalyticsPanel({ locale }: { locale: Locale }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (notificationAddonsResponse.ok) {
        ...
        setNotificationAddonError(notificationAddonResult.error || (zh ? "房源提醒状态暂时无法加载。" : "Listing alert status is unavailable right now."));
      }
    } catch (loadError) {
  ...
  const requestNotificationAddon = async (listingId: string) => {
    ...
    try {
      ...
      if (!response.ok || !result.addon) throw new Error(result.error || (zh ? "房源提醒申请暂时无法提交。" : "Listing alert request could not be submitted."));
      ...
      setNotificationAddonError(requestError instanceof Error ? requestError.message : (zh ? "房源提醒申请暂时无法提交。" : "Listing alert request could not be submitted."));
    } finally {
  ...
  return <section className="analytics-panel" aria-labelledby="analytics-title">
    ...
    {loading ? <div className="dashboard-loading" aria-live="polite"><span /><span /><span /></div> : <>
      ...
      <section className="listing-alert-addon-panel" aria-labelledby="listing-alert-addon-title">
        <div className="listing-alert-addon-heading"><div><span className="section-label">OWNER ALERTS</span><h3 id="listing-alert-addon-title">{zh ? "房源提醒增值功能" : "Paid owner alerts"}</h3><p>{zh ? "付费后，房主可为单套房源接收新咨询、租赁申请、经纪回复和到期提醒。未开通时，房源仍可正常发布和管理。" : "After payment, owners can receive inquiry, application, agent-response, and expiration alerts for an individual listing. Publishing and dashboard access remain available without it."}</p></div><strong>{addonPriceLabel}</strong></div>
