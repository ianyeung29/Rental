...
The posting wizard lets the poster choose up to three nearby lookup categories. The Chinese-community option prioritizes public-transit routes to Downtown Flushing or Brooklyn 8th Avenue, and the transit option finds a nearby subway/bus stop, walking time, and serving line numbers when Google returns them. Clearing all categories skips Google Maps for that polish. Successful results are stored in the Neon `rental_location_context_cache` table for seven days; exact-address lookups use a one-way address hash in the cache key and never store the raw address.
...
- The detail drawer can estimate a route to a school, workplace, landmark, Chinese commercial district, or supermarket from the public approximate area. Public-transit estimates include returned line numbers when available. Listing polish can use the private address server-side for more accurate nearby references without exposing it to AI or the public listing; both flows support drive/public transit/walking and cache non-address results for seven days.
